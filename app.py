...
with tab2:
    ...
    if final_courses:
        ...
        # 최종 판정
        is_pass = all([
            total_sum >= criteria['total_credits'],
            (maj_req + maj_sel) >= criteria['major_total'],
            maj_req >= criteria['major_required'],
            not req_fail
        ])

        st.header("🏁 졸업 자격 진단 리포트")
        if is_pass: st.success("🎉 축하합니다! 모든 졸업 요건을 충족했습니다."); st.balloons()
        else: st.error("⚠️ 아직 충족되지 않은 요건이 있습니다.")

        col1, col2, col3 = st.columns(3)
        col1.metric("총 취득학점", f"{int(total_sum)} / {criteria['total_credits']}")
        col2.metric("전공 합계", f"{int(maj_req + maj_sel)} / {criteria['major_total']}")
        col3.metric("리더십(RC 포함)", f"{leadership_count} / 2")

        if req_fail:
            st.warning(f"**미이수 필수 요건:** {', '.join(req_fail)}")
