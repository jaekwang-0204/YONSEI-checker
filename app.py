...
with tab1:
    img_files = st.file_uploader("이미지 파일 업로드", type=['png','jpg','jpeg'], accept_multiple_files=True)
    if img_files and st.button("🔍 성적 이미지 분석 실행"):
        all_results = []
        with st.spinner(f"{len(img_files)}장의 이미지를 분석 중..."):
            for img in img_files: 
                # 수정된 포인트: selected_full_key 전달
            ...
            if all_results:
                ...
                st.success(f"분석 완료! {len(st.session_state.ocr_results)}개의 강의를 인식했습니다.")
                st.rerun()

with tab2:
    st.markdown("### 📝 수강 강의 관리")
    # 수정된 포인트: selected_full_key 사용
    img_path = f"images/{selected_full_key}_{selected_dept}.png"
    try:
        guide_img = Image.open(img_path)
        st.image(guide_img, caption=f"📖 {selected_full_key} 가이드 이미지", use_container_width=True)
    except FileNotFoundError:
        # 괄호 제거한 기본 이미지 파일명으로도 시도 (예비책)
        basic_path = f"images/{selected_year_num}_{selected_dept}.png"
        try:
            st.image(Image.open(basic_path), caption=f"📖 {selected_year_num} 가이드 이미지", use_container_width=True)
        except:
            st.caption("ℹ️ 가이드 이미지가 없습니다.")
    ...
    if final_courses:
        ...
        for row in final_courses:
            ...
            is_exactly_basic = any(basic in c_name for basic in basic_list) or (c_name == "조직학")
        ...
        for item in gen.get("required_courses", []):
            ...
            elif not any(normalize_string(kw) in normalize_string(search_names) for kw in item["keywords"]):
        ...
        st.info("ℹ️ 본 진단 결과는 참고용이며, 학과 사무실을 통해 최종확인하시기 바랍니다.")
        st.header("🏁 졸업 자격 예비진단 리포트")
        if is_all_pass: 
            st.success("🎉 모든 요건을 충족했습니다."); st.balloons()
        else: 
            st.error("⚠️ 미충족 요건이 있습니다.")
        ...
        m4.metric("리더십", f"{leadership_count}/2")
